...
| V2.3 | 2026-07-23 | 架构评审修订：Token 安全、MailDelivery 状态、Batch Lease、邮件 Snapshot、健康检测、SPF、DKIM 等 |
...
## 4.2 Runner Token 安全下发（V2.3 关键修订）
...
class MailRunner(Base):
    ...
    # 注意：V2.3 不再使用 current_batch_id 表示唯一 Batch
    # 改为支持多个并行 Batch
...
## 5.5 RunnerHealth（窗口数据，V2.3 修订）

**V2.3 重点修订：区分收件人错误和 SMTP 拒绝**
...
class MailBatch(Base):
    ...
    lease_expires_at: datetime | None  # V2.3 新增
    started_at: datetime | None
    completed_at: datetime | None
...
## 5.8 MailDelivery 模型（V2.3 关键修订）

**V2.3 最重要的修订之一：增加 ATTEMPTING 状态和唯一约束**
...
### 8.3 Active IP Set（V2.3）

GitHub Hosted Runner IP 生命周期短，下线后 IP 保留 30 分钟：

- `runner_count`：活跃引用计数
- `retained_count`（V2.3 新增）：Grace Period 保留计数
- `remove_after`：Grace Period 到期时间
- 如果 `runner_count + retained_count = 0` → 从 SPF 删除
- 最大 50 个 IP，超出优先删除最老的
...
|------|------|:--:|
| 方案 A | Oracle 预签名 .eml | — |
| 方案 B | Runner 传 Hash → Oracle 返回 Signature | ✅ V2.3 |
...
Request: {domain, selector, headers[], body_hash}
Response: {dkim_signature}

## 7.5 模型

class DkimKey: domain, selector, private_key(加密), public_key, active
