# Mail Relay Platform + Multi Mail Runner — 实施设计文档 V2.5（架构冻结版本）
...
class MailDeliveryAttempt(Base):
    ...
    batch_id: UUID
    runner_id: UUID
    attempt_number: int          # Oracle 生成，非 Runner 提交（V2.5）
...
# 6. Billing Boundary 定义（V2.5 P0 修正）

**Billing Boundary = TCP connection successfully established with target SMTP server。**

V2.4 的矛盾：CREATED -> CONSUME -> STARTED -> TCP connect，但又说 "TCP connect 失败不产生 Attempt"，实际上 CONSUME 已经执行。
...
STARTED -> CONSUME 1
    |
    +-- EHLO/MAIL FROM/RCPT TO/DATA（无论成败，均计费）
```
...
2. DNS lookup
   失败 -> status=FAILED, 不计费
3. TCP connect
   失败 -> status=FAILED, 不计费
4. TCP connect 成功
   a. status=STARTED
   b. 原子 UPDATE quota_accounts SET reserved-=1, consumed+=1 WHERE reserved>=1
   c. 如果 affected_rows == 0 -> QUOTA_INSUFFICIENT，不允许 SMTP
   d. 如果 affected_rows == 1 -> INSERT QuotaTransaction (CONSUME 1, attempt_id)
   e. 执行 SMTP（EHLO/MAIL FROM/RCPT TO/DATA）
```
...
| # | 修订 | 优先级 | 说明 |
|---|------|:------:|------|
| 1 | Billing Boundary = TCP connect 成功 | P0 | 解决 CONSUME 与计费边界矛盾 |
| 2 | QuotaTransaction UNIQUE(attempt_id, type) | P0 | 幂等约束，防止重复扣费 |
| 3 | RECOVERING 状态层级修正 | P0 | NOT_ATTEMPTED/CREATED/STARTED 分开处理 |
| 4 | MailDeliveryAttempt UNIQUE(delivery_id, attempt_number) | P0 | 保证 attempt_number 唯一，Oracle 生成 |
| 5 | RETRY_EXHAUSTED + 业务/技术状态分层 | P1 | MailDelivery vs Attempt 状态分离 |
| 6 | SPF active_runner_count 原子 UPDATE | P1 | 禁止应用层读-改-写 |
| 7 | claim-next 健康检查 | 文档 | 根据 Runner 健康状态限制分配 |
| 8 | error_type 枚举 | 文档 | 区分收件人错误和 SMTP 拒绝 |
| 9 | DKIM 签名后不可变性 | 文档 | 签名后禁止修改 Body |
| 10 | 核心业务不变量 | 文档 | 四个不变量声明 |
...
**V2.5 是架构冻结版本。此后不再进行架构级别的修改，直接进入实施阶段。**

*文档版本 V2.5（架构冻结版本）— 2026-07-23*
